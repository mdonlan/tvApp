<template>
  <div class="popularContainer">
    ...
    <div class="airingTodayShows">
      <div class="showContainer" v-for="show in popularShows" v-on:click="goToShowDetails($event, show)">
...
export default {
  ...
  methods: {
    getPopular() {
      ...
      axios({
        method:'get',
        url:'https://api.themoviedb.org/3/discover/tv?api_key=75234636e15f7c2463efbf69fd35b291&language=en-US&sort_by=popularity.desc&page=1&timezone=America%2FNew_York&include_null_first_air_dates=false&page=' + self.onPage,
      })
      .then(function(response) {
        ...
        if(self.popularShows) {
          let oldShows = self.popularShows;
          let newShows = data.results;
          newShows.forEach(function(show) {
            oldShows.push(show);
          }) 
          self.popularShows = oldShows;
        } else {
          self.popularShows = data.results;
        }
        
        // if not all pages have loaded keep loading them
        if(self.onPage < self.totalPages && self.onPage < 10) {
